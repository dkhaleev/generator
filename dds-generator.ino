/* Simple DDS generator for arduino Uno (mega238 based) v 2.3
 * Pinout: Rotary encoder EC-11 A0,A1 (in)
 * Rotary encoder button A3 (in)
 * Mode button A2 (in)
 * LCD backlight 8 (out) (вывод BL дисплея)
 * LCD nokia 5110 - 13,12,11,10 (out)  
 * Generator uotput 0..7 + 9 (out)
 */ 


const PROGMEM uint8_t  sinewave[]= // sine wave array
{

};

const PROGMEM uint8_t squarewave[]= //
{
,
};

const PROGMEM uint8_t trianglewave[]= //массив треугольника
{

};

const PROGMEM uint8_t sawtoothwave[]= //массив пила1
{

};

const PROGMEM uint8_t rewsawtoothwave[]= //массив пила2
{
,
};


const PROGMEM char musor_mass[]= //массив для подгонки адреса в флэш-памяти
{
,
};


volatile int32_t freq=1000;// default frequency по умолчанию
volatile uint16_t enc_step=100; //default rotary encoder step
volatile uint8_t mode=5; //mode by default
volatile uint8_t monitor_flag; // LCD output flag
volatile uint8_t ad2,ad1,ad0; //service bytes for asm parts
volatile uint8_t duty=50; //duty cycle percent by default
volatile uint32_t icr=0;  //timer1 comparator variable

#include <LCD5110_SSVS.h>
extern uint8_t SmallFont[];
extern uint8_t MediumNumbers[];
LCD5110 lcd(10,11,12,13);

// DDS algorithm 
static inline void signalOUT(const uint8_t *signal){ 
   asm volatile(  "eor r18, r18   ;r18<-0"  "\n\t"
    "eor r19, r19   ;r19<-0"  "\n\t"
     "1:"            "\n\t"
      "add r18, %0  ;1 cycle"     "\n\t"
       "adc r19, %1  ;1 cycle"     "\n\t"  
        "adc %A3, %2  ;1 cycle"     "\n\t"
         "lpm      ;3 cycles"  "\n\t"
         "out %4, __tmp_reg__  ;1 cycle" "\n\t"
        "sbis %5, 7   ;1 cycle if no skip" "\n\t"     
       "rjmp 1b    ;2 cycles. Total 10 cycles" "\n\t" 
      :
     :"r" (ad0),"r" (ad1),"r" (ad2),"e" (signal),"I" (_SFR_IO_ADDR(PORTD)), "I" (_SFR_IO_ADDR(PORTB))
    :"r18", "r19" );
    }

void setup(){
lcd.InitLCD();
for (int n=0; n <sizeof(musor_mass); n++ ) {PORTD=musor_mass[n]; }   
 PORTC|=(1<<PC2)|(1<<PC3); //buttons A2 A3 pullup
  TCCR0B=0; TCCR1A=0; TCCR1B=0;
  DDRB|=1<<PB0; // 8 pin - disable backlight  = exit
   PCICR=(1<<PCIE1); //enable PCINT interruption
    PCMSK1=(1<<PCINT9)|(1<<PCINT10)|(1<<PCINT11); //select interruption pins (A1,A2,A3)
      check_mode(); 
         } //end setup


void up_down(boolean x){ // settings management

if (TCCR1B==17 && ICR1<2800 && mode==5){ 
if(x) { if (icr<1000 && enc_step > 100) enc_step=100; 
        if (icr<100 && enc_step > 10) enc_step=10;
        if (icr<10 && enc_step > 1) enc_step=1;          
         icr-=enc_step;
          if (icr<2) icr=2;    }
       else {  if (icr > 1800 && enc_step >100) enc_step =100;
          icr+=enc_step ;   } 
               return; 
            }
if (mode==6){ 
 
        if (ICR1>100){ 
             
              if (x){if(duty<100) {duty++; }  }
             
              if (!x){ if(duty>0) {duty--; }}
               }
         else{ 
                if (x){if(OCR1A<ICR1) {OCR1A++; }  }         
                 else {if(OCR1A>0) {OCR1A--; }  }         
               if(OCR1A>ICR1) OCR1A=ICR1-1;
                duty=(uint32_t)100*OCR1A/ICR1;                 
         }
     return;
  } 

x? freq+=enc_step : freq-=enc_step ;  
if (freq < 1) freq=1;
}

        
        
// interruption from encoder and buttons         
        ISR (PCINT1_vect){ 
        PCMSK1=0; PCICR=0; 
        PORTD=0;
   // if encoder button pressed
  if ((PINC&(1<<3))==0){ 
        while ((PINC&(1<<3))==0);// wait untill button is released

      //LCD backlight on/off (8 pin)
        if ((PINC&(1<<2))==0){ //if mode button is pressed after,
         PINB|=1<<PB0; //change state of 8th pin
          while ((PINC&(1<<2))==0);// now wait until mode button is released 
          PCMSK1=(1<<PCINT9)|(1<<PCINT10)|(1<<PCINT11); PCICR=(1<<PCIE1); return;
          }
     if (mode==6) { mode=5; check_mode(); PCMSK1=(1<<PCINT9)|(1<<PCINT10)|(1<<PCINT11); PCICR=(1<<PCIE1); return; }
          switch (enc_step){ 
                  case 1: enc_step=10;     break;
                   case 10: enc_step=100;    break;
                   case 100: enc_step=1000;   break;
                   case 1000: enc_step=1;  
                  }
               check_mode();
              PCMSK1=(1<<PCINT9)|(1<<PCINT10)|(1<<PCINT11); PCICR=(1<<PCIE1); return;
            } 
if ((PINC&(1<<2))==0){ // if mode button is pressed
   PORTD=0;
    while ((PINC&(1<<2))==0);
     mode++; if (mode==7) mode=0; 
     check_mode(); PCMSK1=(1<<PCINT9)|(1<<PCINT10)|(1<<PCINT11);PCICR=(1<<PCIE1);  return;
      }

// if buttons aren't pressed, encoder state was changed:
        up_down(  (PINC&1)^((PINC&2)>>1) ); //send output from encoder to separate function
        // if counting for rotation goes in wrong direction, inverse  '!'   up_down(! 
        check_mode(); 
          PCMSK1=(1<<PCINT9)|(1<<PCINT10)|(1<<PCINT11); PCICR=(1<<PCIE1);//включить прерывания

         } //end of interruption processing function for buttons PCINT1_vect 


void pwm_gen(){ //timer1 setup 
 uint16_t divider=1; 
  icr = (16000000ul / freq /2 /divider); 
   byte shifts[] = {3,3,2,2};
     for(byte i = 0; i < 4; i++){
        if (icr > 65536) {
           divider <<= shifts[i];
           icr = 16000000ul / freq /2 /divider;
           }
    else {  
        TCCR1B = (i+1)|(1<<WGM13);  break;
         }
    }
 ICR1=icr-1; 
set_duty();
} //end pwm_gen

    
void loop() {
   if (monitor_flag) { monitor_flag=0; monitor_out(); }
               if (mode <5){ PORTB&= ~(1<<7);
               DDRD=0xFF;//set D port as output
               uint32_t temp=(float)freq /0.095367431640625; 
               ad2=temp>>16; ad1=temp>>8; ad0=temp;
                 switch (mode){ 
                  case 0: signalOUT(sinewave); break;
                   case 1: signalOUT(trianglewave); break;
                    case 2: signalOUT(squarewave); break;
                     case 3: signalOUT(sawtoothwave); break;
                      case 4: signalOUT(rewsawtoothwave);
                       } //end switch
                     DDRD=0; PORTD=0;
              
                } //end if (mode<5)

 }


void monitor_out(){
String dutystr,stepstr;
dutystr= String("Duty="+ String(duty)+ "%");                
stepstr=String("Step= "+String(enc_step));
lcd.clrScr();
//1st row output
lcd.setFont(SmallFont);
if (freq <10000) {lcd.print("Frequency, Hz",LEFT,0 );  }
if (freq >=10000) {lcd.print("Frequency, kHz",LEFT,0 );  }
//2nd row output
lcd.setFont(MediumNumbers);
if (freq <10000) { lcd.printNumI(freq, CENTER, 8); }
if (freq >=10000u && freq < 1000000ul ) { lcd.printNumF( ((float)freq/1000),3 ,CENTER, 8);  }
if (freq >=1000000ul ) { lcd.printNumF( ((float)freq/1000),2 ,CENTER, 8);  }
//3rd row output
lcd.setFont(SmallFont);
     switch(mode){ 
       case 0: lcd.print("Sinus DDS",CENTER, 32); break;
        case 1: lcd.print("Triangle DDS",CENTER, 32); break;
         case 2: lcd.print("Meandr DDS",CENTER, 32); break;
        case 3: lcd.print("Pila1 DDS",CENTER, 32); break;
       case 4: lcd.print("Pila2 DDS",CENTER, 32); break;
      case 5: lcd.print("PWM Mode",CENTER,32); break;
    case 6: lcd.print("Duty Mode", CENTER, 32);
     }
//4th row output
 if (mode==6) lcd.print(dutystr, CENTER, 40);
         else lcd.print(stepstr, CENTER, 40);
}//end monitor out


void set_duty(){
//float ocr,delta_ocr;
if (mode==6&&ICR1<100) return;
if (mode==5 && ICR1<100){
    OCR1A=ICR1/2; duty=50; 
    return;
     }
static uint16_t ocr;
ocr=(uint32_t)ICR1*duty/100;
OCR1A=ocr;
}

void check_mode(){// generator check and mode setup 
if (mode <5){ if (freq > 100000) freq=100000;
    TCCR1B=0;
    TCCR1A=0;  DDRB&=~(1<<DDB1); // 9 pin arduino set Z-mode 
    }// end if mode <5

  if (mode > 4) { 
      TCCR1A=1<<COM1A1; DDRB|=1<<DDB1; // 9 pin set output (pwm out)
      if (TCCR1B==17 && ICR1<2800){ 
              ICR1=icr; freq= (float) 8000000UL/ICR1;  
              set_duty();  
                    }
                 else { pwm_gen(); } 
         }
   PORTB|= 1<<7; 
monitor_flag=1;
}
